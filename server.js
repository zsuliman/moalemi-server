...
4. الطقس: إذا سأل عن الطقس ولم تتوفر المعلومة، اطلب منه ذكر اسم مدينته.${weatherSecretContext}`;
    ...
    if (question) {
      ...
      currentParts.push({ text: "اشرح لي هذه الصورة." });
    }

    if (imageBase64) {
      // الحل الجذري هنا: كتابة المتغيرات بالشكل الذي تقبله جوجل حصراً
      currentParts.push({
        inlineData: {  
          mimeType: "image/jpeg", 
...
app.listen(PORT, () => { console.log(`🚀 Server running on port ${PORT} with Gemini Flash 1.5 (Final Fix)`); });